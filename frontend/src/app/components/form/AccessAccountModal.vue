  ...
  <div class="modal" v-if="visible">
    <div class="modal-content p-6 bg-gray-800 text-white rounded-lg shadow-lg w-full max-w-md">
      ...
      <form @submit.prevent="handleSubmit">
        <div class="space-y-6">
          <div>
            <label for="accountNumber" class="block text-sm font-medium text-gray-300">Número da Conta</label>
            <input
              id="accountNumber"
        ...
        <div class="flex justify-end mt-6 space-x-4">
          ...
          <button
            type="submit"
            class="px-6 py-2 bg-green-500 text-white rounded-lg hover:bg-green-400 transition"
          >
            {{ submitText }}
          </button>
        </div>
...
const emit = defineEmits<{
  (e: 'submit', accountNumber: string): void;
  (e: 'close-modal'): void;
}>();
...
const handleSubmit = () => {
  emit('submit', accountNumber.value);
};
</script>
